...
def _get_python_inc_from_config(plat_specific, spec_prefix):
    ...
    directory from the config.py vars. Useful when
    cross-compiling, since the config.py vars may come from
    the host
...
def customize_compiler(compiler):  # noqa: C901
    ...
    if compiler.compiler_type == "unix":
        if sys.platform == "darwin":
            ...
            # config.py vars on OS X now that we know we need a compiler.
...
def parse_config_h(fp, g=None):
    """Parse a config.py.h-style file.
